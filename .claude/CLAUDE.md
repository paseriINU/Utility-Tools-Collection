...
### タスク完了時のコミット（セッション単位でまとめる）

**重要**: セッション中は**1つのフィーチャーブランチに変更をまとめる**こと。

1. **セッション開始時（最初の変更時）**
   - フィーチャーブランチを作成: `claude/session-[タイムスタンプ]`
   - 例: `claude/session-1765921204`

2. **変更ごとにコミット**
   - 各変更はコミットするが、PRは作成しない
...
4. **ユーザーが「PRして」「マージして」と指示した時**
   - リモートにプッシュ: `git push -u origin [ブランチ名]`
   - PRを作成: `gh pr create`（全コミットをまとめたSummaryを作成）
   - PR URLをユーザーに報告

**例**:
```
[最初の変更]
フィーチャーブランチを作成してコミットします...
...
- mainブランチへの直接プッシュは禁止
- 必ずPRを通してマージすること
- セッション終了時にPR未作成の変更がある場合は、ユーザーに確認すること
