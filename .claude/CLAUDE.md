...
**方針**: 複雑な処理を実装する場合は、**車輪の再発明を避け、信頼性の高いライブラリを使用**すること。

1. **ライブラリを検討すべきケース**
   - バイナリデータの解析（PDF、画像、圧縮ファイル等）
   - 暗号化・ハッシュ処理
   - 日付・時刻の複雑な操作
   - 文字エンコーディング変換
   - 複雑な正規表現パターン
...
2. **推奨されるライブラリ例**
   - **PDF解析**: PDF.js（Mozilla製）
   - **圧縮/解凍**: pako（zlib）
   - **日付処理**: Day.js、date-fns
   - **暗号化**: Web Crypto API（標準）

3. **HTMLツールへの埋め込み方法**
   - インターネット不可環境を考慮し、**CDNではなくHTMLに直接埋め込む**
   - ライブラリのminified版を使用してサイズを最小化
   - ライセンス表記を含める
   - 例: `<!-- ライブラリ名: バージョン - ライセンス -->` + `<script>...</script>`

4. **フォールバック設計**
   - ライブラリが利用できない場合の代替処理を用意
   - 例: `if (typeof libraryName !== 'undefined') { /* ライブラリ使用 */ } else { /* フォールバック */ }`

**例**:
```javascript
// PDF.jsが利用可能な場合はそれを使用
...
3. **タスク完了時（ghが利用可能な場合は自動マージ）**
   - `gh` コマンドが利用可能な環境では、タスク完了時に自動的にマージまで実行する
   - PRをマージ: `gh pr merge --merge --delete-branch`
   - mainブランチに切り替え: `git checkout main && git pull`
   - ユーザーへの確認は不要（ghが使える＝マージまで完了させる）
...
- mainブランチへの直接プッシュは禁止
- 必ずPRを通してマージすること
- ghが利用可能な場合はタスク完了時に自動マージ（確認不要）
- ghが利用できない環境ではPR作成までで完了とし、ユーザーに手動マージを依頼
...
- **文字エンコーディング（必須）**:
  - VBAファイル（.bas）はShift_JIS（SJIS）で保存すること
  - SJISで表現できない文字は使用しないこと
  - 特殊記号（チェックマーク、絵文字等）は避け、代替表現を使用
  - 例: 「○」「×」「※」などはSJISで表現可能
...
- **モジュール名の命名規則（必須）**:
  ...
  - スペースや特殊文字は使用不可
  - 予約語（Sub、Function、Dim、Moduleなど）は使用不可
  - **最大31文字まで**（これを超えるとエラー）
  - 例: `WBO_Config`, `JP1_JobManager_Setup`, `CommonUtils`

- **モジュール分離（必須）**:
  - **小規模ツール**: 最低2つのモジュールに分離
    1. **初期化モジュール** (`[略称]_Setup.bas`): シート作成・フォーマット処理
    2. **メインモジュール** (`[略称]_Main.bas`): ビジネスロジック・実行処理

  - **大規模ツール（推奨構成）**: 責務ごとに細分化
    1. **Config** (`[略称]_Config.bas`): 定数、型定義、設定読み込み
    2. **Main** (`[略称]_Main.bas`): エントリーポイント、各モジュール呼び出し
    3. **Processor** (`[略称]_Processor.bas`): 中核処理ロジック
    4. **Ops/Utils** (`[略称]_[機能名].bas`): 機能別ユーティリティ
    5. **Setup** (`[略称]_Setup.bas`): シート作成のみ（削除可能に設計）

  - **Setupモジュールを削除可能にする設計**:
    - 定数はConfigモジュールで定義（Setupは参照のみ）
    - シート作成後はSetupを削除してもメイン処理が動作すること
    - 初期化は最初の1回だけ実行するため、不要になったら削除可能

  - **モジュール間の依存関係**:
    - Main → Config, Processor, Ops（呼び出し）
    - Processor → Config, Ops（参照）
    - Setup → Config（定数参照のみ）
    - Ops → Config（定数参照）
  ...
  - 例（大規模ツール）:
    ```
    Word_しおり整理ツール/
    ├── WBO_Config.bas      # 定数・型・設定読み込み
    ├── WBO_Main.bas        # エントリーポイント
    ├── WBO_Processor.bas   # 段落処理ロジック
    ├── WBO_WordOps.bas     # Word操作ユーティリティ
    └── WBO_Setup.bas       # シート作成（削除可能）
    ```

  - 例（小規模ツール）:
    ```
    JP1_ジョブ管理ツール/
    ├── JP1JM_Setup.bas     # 初期化モジュール
    └── JP1JM_Main.bas      # メインモジュール
    ```
