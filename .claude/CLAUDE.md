...
2. **更新対象のREADME（すべて必須）**
   ...
   - **ルートREADME（必須）**: `README.md`（リポジトリルート）
     - 新規ツール追加時: フォルダ構成とツール一覧に追加
...
**重要**: コード修正タスクが完了したら、**自動的にコミット・プッシュ・PR作成**を行うこと。

1. **フィーチャーブランチを作成**
   - 命名規則: `claude/[機能名]-[タイムスタンプ]`
   - 例: `claude/linux-deploy-cleanup-feature-1765921204`
...
5. **PR URLをユーザーに報告**
   - 作成したPRのURLを必ずユーザーに伝える

**例**:
```
[タスク完了後]
フィーチャーブランチを作成してプッシュします...
