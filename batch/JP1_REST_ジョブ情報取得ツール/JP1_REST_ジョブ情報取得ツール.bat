...
# ------------------------------------------------------------------------------
# ★ 注意: ジョブパス指定方式では、この設定を変更しても効果はありません。
#         内部で "NO"（直下のみ）が使用されます。
#
...
# ------------------------------------------------------------------------------
# ★ 注意: ジョブパス指定方式では、この設定を変更しても効果はありません。
#         ジョブ名で完全一致フィルタするため、unitTypeフィルタは使用されません。
#
# 取得するユニットの種別を指定します。
#
# 指定可能な値:
...
# ------------------------------------------------------------------------------
# ユニット名フィルタ（参考情報 - 現在未使用）
# ------------------------------------------------------------------------------
# ★ 注意: この設定は現在使用されていません。
#         ジョブパスから自動的にジョブ名を抽出して完全一致（EQ）フィルタを行います。
#
# 【参考】ユニット名の比較方法（(3) MatchMethods）
#   "NO" - この比較方法を検索条件にしません
#   "EQ" - 完全一致
#   "BW" - 前方一致（例: "daily*"）
#   "EW" - 後方一致（例: "*_backup"）
