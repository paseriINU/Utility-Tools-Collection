...
2. ダブルクリックで実行
3. 結果が `プレフィックス＋ジョブ開始日時＋サフィックス.txt` 形式で保存されます
4. ファイル名はバッチ内の `FILE_PREFIX` と `FILE_SUFFIX` で変更可能です
   - デフォルト例: `テスト（20251010_163520実行分）.txt`
5. ファイル名にジョブの実際の開始日時が使用されるため、履歴管理が容易です
6. **警告機能**: ジョブ開始日時が2日以上前の場合、警告を表示して確認を求めます
