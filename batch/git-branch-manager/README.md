...
```
git-branch-manager/
├── delete_branches.bat         # 統合版（すべての機能）
├── delete_remote_branch.bat    # リモートブランチ削除（シンプル版）
├── delete_local_branch.bat     # ローカルブランチ削除（シンプル版）
└── README.md
```
...
[1] origin/feature/login
[2] origin/feature/payment
[3] origin/hotfix/bug-123

[0] キャンセル

削除するブランチ番号 (1-3, 0=キャンセル): 1

ブランチ: origin/feature/login
...
```
1. delete_remote_branch.bat を実行
2. 削除したいブランチを番号で選択
3. [Y] で確認
```
