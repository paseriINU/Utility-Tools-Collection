...
JP1/AJS3のジョブネット起動とログ取得を行う統合ツールです。

## 概要

以下の機能をメニュー形式で提供します：

1. **ジョブネット起動** - ajsentryでジョブネットを即時起動（完了待ち付き）
2. **ジョブログ取得** - ajsshowでジョブの標準出力を取得しクリップボードにコピー
3. **ジョブネット起動 + ログ取得** - 起動→完了待ち→ログ取得を連続実行
...
| ツール | 説明 | 用途 |
|--------|------|------|
| [JP1 ローカルジョブ起動ツール](../JP1_ローカルジョブ起動ツール/) | ジョブネット起動のみ | シンプルな起動のみ |
| [JP1 ジョブログ取得ツール](../JP1_ジョブログ取得ツール/) | ログ取得のみ | シンプルなログ取得のみ |
| [JP1 リモートジョブ起動ツール](../JP1_リモートジョブ起動ツール/) | リモートから起動 | クライアントPCから実行 |
