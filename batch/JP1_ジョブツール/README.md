...
また、3つの標準ログバッチを使って簡単に呼び出すことができます。
...
| ファイル | 説明 |
|---------|------|
| `JP1ジョブ実行.bat` | ジョブを即時実行してログを取得（メインツール） |
| `JP1ジョブログ取得.bat` | 実行せずに既存の結果を取得（メインツール） |
| `JP1ジャーナル取得.bat` | 標準ログバッチ（EXEC/GET切替、Excel貼り付け対応） |
| `【JP1ジョブ実行】ジョブ.bat` | 標準ログバッチ（実行+ログ取得専用） |
| `【JP1ジョブログ取得】ジョブ.bat` | 標準ログバッチ（ログ取得専用） |
| `README.md` | このファイル |
