...
以下のブランチは**一覧に表示されず、選択できません**：

- `main`
- `master`

これらのブランチはリモート・ローカル・両方削除のいずれの機能でも表示されないため、誤って削除することを防止します。

---

### 現在のブランチの保護

ローカルブランチ削除時、現在チェックアウトしているブランチを選択すると、別のブランチへの切り替えを促されます。
...
# 保護ブランチリスト（削除不可）
$ProtectedBranches = @("main", "master")  # 追加可能: "develop", "production" など
#endregion
```
