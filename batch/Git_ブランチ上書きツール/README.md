...
5. **リモートプッシュを選択**
   - リモートにプッシュする（`--force-with-lease`を使用）
   - ローカルのみの変更も可能
   - ※ `--force-with-lease`が失敗した場合のみ`--force`を提案
...
| オプション | 説明 |
|----------|------|
| `--force-with-lease` | 標準で使用。他の人がプッシュした場合は失敗し、安全 |
| `--force` | `--force-with-lease`失敗時のみ提案。他の人の変更も上書き |
| ローカルのみ | リモートにプッシュしない |
...
2. **プッシュオプション**:
   - `--force-with-lease`を標準で使用。他の人がプッシュした場合は失敗し、安全です。
   - `--force`は`--force-with-lease`失敗時のみ提案されます。他の人の変更も上書きするため、使用時は注意してください。
