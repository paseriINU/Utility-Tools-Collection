...
| オプション | 説明 |
|----------|------|
| `--force-with-lease` | 推奨。他の人がプッシュした場合は失敗し、安全 |
| `--force` | 最終手段。他の人の変更も上書きするため注意が必要 |
| ローカルのみ | リモートにプッシュしない |
...
2. **プッシュオプション**:
   - `--force-with-lease`（推奨）: 他の人がプッシュした場合は失敗し、安全です。
   - `--force`（最終手段）: 他の人の変更も上書きするため、チームで使用する場合は事前に連絡してください。
