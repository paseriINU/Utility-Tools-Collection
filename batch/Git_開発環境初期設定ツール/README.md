...
7. **WinMergeフィルター設定**（WinMergeがインストールされている場合）
   ...
   【フィルターの適用方法（初回のみ）】
     以下の手順でフィルターをWinMergeにインストールしてください：

     1. WinMergeを起動
     2. メニューから [ツール] > [フィルター] を選択
     3. [ファイルフィルター] タブの [インストール] ボタンをクリック
     4. 以下のパスを開き、GitFiles.flt を選択して [開く]

        C:\Users\tanaka\AppData\Roaming\WinMerge\Filters\GitFiles.flt

     5. 一覧に「Git管理ファイル除外フィルター」が表示されたら [OK]
     6. フォルダ比較を実行
   ```
