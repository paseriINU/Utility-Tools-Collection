...
"@
        ...
        Write-Host ""
        Write-Color "【フィルターの適用方法（初回のみ）】" "Yellow"
        Write-Host "  以下の手順でフィルターをWinMergeにインストールしてください："
        Write-Host ""
        Write-Host "  1. WinMergeを起動"
        Write-Host "  2. メニューから [ツール] > [フィルター] を選択"
        Write-Host "  3. [ファイルフィルター] タブの [インストール] ボタンをクリック"
        Write-Host "  4. 以下のパスを開き、GitFiles.flt を選択して [開く]"
        Write-Host ""
        Write-Host "     $filterFile"
        Write-Host ""
        Write-Host "  5. 一覧に「Git管理ファイル除外フィルター」が表示されたら [OK]"
        Write-Host "  6. フォルダ比較を実行"
    }
    Write-Host ""
}
#endregion
