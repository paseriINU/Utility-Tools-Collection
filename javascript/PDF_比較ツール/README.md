...
| ファイル | 比較方式 | 用途 |
|---------|---------|------|
| `pdf_compare.html` | テキスト比較（WinMerge方式） | テキストベースPDFの内容比較 |
| `pdf_visual_compare.html` | 視覚比較（並列表示） | レイアウト・画像含む見た目の比較 |
...
### テキスト比較（pdf_compare.html）
- **高精度な差分検出** - LCS（最長共通部分列）アルゴリズムを使用
- **WinMerge方式の表示** - 旧/新ファイルの行番号を両方表示
- **CSVエクスポート** - 比較結果をCSVで出力可能

### 視覚比較（pdf_visual_compare.html）
...
**結果のエクスポート**: 比較完了後、「CSVエクスポート」ボタンをクリックすると、結果をCSVファイルとしてダウンロードできます。

### 視覚比較（pdf_visual_compare.html）

1. `pdf_visual_compare.html` をダブルクリックしてブラウザで開く
2. 「旧ファイル (左)」エリアに比較元のPDFをドラッグ&ドロップ
3. 「新ファイル (右)」エリアに比較先のPDFをドラッグ&ドロップ
4. 両方のPDFが読み込まれると自動的に比較開始
5. 左右にPDFが並列表示され、差分が色付きでハイライト

**操作方法**:
- **ページ移動**: 「前のページ」「次のページ」ボタン、またはページ番号入力
- **ズーム**: 「−」「+」ボタンで50%〜200%の範囲で拡大・縮小
- **スクロール**: 左右のビューは同期してスクロール
- **差分確認**: 色付きの領域が差異箇所（黄=変更、緑=追加、赤=削除）
...
```
PDF_比較ツール/
├── pdf_compare.html        ← テキスト比較ツール
├── pdf_visual_compare.html ← 視覚比較ツール
├── README.md
└── lib/
