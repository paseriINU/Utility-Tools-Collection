...
- **解析モード選択**: Word版/JavaScript版の2つの解析モードに対応
...
PDFファイルを選択すると、解析モード選択ボタンが表示されます。しおりの作成方法に応じて適切なモードを選択してください：

| モード | 説明 | 解析方法 |
|--------|------|----------|
| **📄 Word版** | Word文書から変換されたPDFのしおりを解析 | extractBookmarks関数を使用（従来方式） |
| **📜 JavaScript版** | JavaScript APIで作成されたしおりを解析 | ObjStm（Object Streams）のみを使用 |

**選択のポイント:**
- Word/ExcelでPDF変換した場合 → **Word版**を選択
- Adobe AcrobatのアクションやJavaScriptでしおりを追加した場合 → **JavaScript版**を選択
- JavaScript版はObjStmからの抽出を強制するため、対応していないPDFではしおりが取得できません
- 迷った場合はまず**Word版**を試してください

### 4. 検証実行

解析モードを選択後、「しおり検証」ボタンをクリックすると、PDFを解析してしおり一覧を表示します。
...
| 作成方法 | 対応状況 |
|----------|----------|
| Word/Excel から直接PDF出力 | ○ 対応 |
| Adobe Acrobat でしおり追加 | ○ 対応 |
| Adobe Acrobat JavaScript API | ○ 対応 |
| PDF編集ソフト（PDFelement等） | ○ 対応 |
| Ghostscript等での変換 | ○ 対応 |

### 制限事項

- **暗号化PDF**: パスワード保護されたPDFは解析できません
- **特殊な圧縮形式**: LZWDecode等、FlateDecode以外の圧縮形式には非対応
...
→ 以下の原因が考えられます：
- PDFが暗号化されている
- LZWDecode等のFlateDecode以外の圧縮形式が使用されている
- PDFが破損している
