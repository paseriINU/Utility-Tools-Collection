...
| ファイル | 説明 | WinMerge起動 |
|----------|------|-------------|
| `PDFテキスト抽出ツール.html` | ブラウザ版（推奨） | 手動 |
| `PDFテキスト抽出_WinMerge起動.hta` | HTA版（Windows専用） | 自動 |
...
**注意**: HTA版はIE11ベースのため、PDF.jsの動作が不安定な場合があります。その場合はHTML版をご使用ください。
...
```
PDF_テキスト抽出ツール/
├── PDFテキスト抽出ツール.html        ← ブラウザ版（推奨）
├── PDFテキスト抽出_WinMerge起動.hta  ← HTA版（WinMerge自動起動）
└── README.md
```
