...
|----------|------|
| `WBO_Config.bas` | 定数・型定義・設定読み込み |
| `WBO_Main.bas` | エントリーポイント（各モジュール呼び出し） |
| `WBO_Processor.bas` | 段落処理ロジック |
| `WBO_WordOps.bas` | Word操作ユーティリティ |
| `WBO_Setup.bas` | シート作成・初期化（削除可能） |
| `README.md` | このファイル |

## 更新履歴

- **2026-01-15**: モジュール構造のリファクタリング
  - 5つのモジュールに分割（Config, Main, Processor, WordOps, Setup）
  - 設定シートを追加し、スタイル設定を動的に管理可能に
  - Setupモジュールを削除可能に設計（シート作成後は不要）
