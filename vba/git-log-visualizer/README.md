...
- **処理時間**: コミット数が多い場合、処理に時間がかかる場合があります
- **既存シート上書き**: 実行するたびに4つのシート（Dashboard、CommitHistory、Statistics、Charts）が上書きされます
...
- **2025-12-07**: 初版リリース
  - Git Log 可視化機能を実装
  - 4つのシート（Dashboard、CommitHistory、Statistics、Charts）を作成
  - 全ブランチ対応
  - 作者別・日別統計機能
  - グラフ可視化機能
