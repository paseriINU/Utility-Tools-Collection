...
| `-f フォーマット` | 指定フォーマットで実行結果情報を出力 | -s,-p,-l,-i,-r,-xw,-T |
| `-i フォーマット` | 指定フォーマットで結果出力（シングルクォートで囲む） | -s,-p,-l,-f,-r,-xw,-T |
...
rem 配下のユニットも含めて状態取得
ajsshow -R -f "%JJ %TT %CC %RR" /main_unit/jobgroup1
