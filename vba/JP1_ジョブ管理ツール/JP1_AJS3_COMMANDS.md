...
| `-p パスワード` | JP1パスワードを指定 |
| `-E` | 実行結果の詳細情報を取得 |
| `-i フォーマット` | 指定したフォーマットで定義情報を出力 |
| `-g 世代番号` | 取得する実行世代を指定（1=最新、a=全て） |
| `-R` | 配下のユニットも再帰的に取得 |
| `-T` | ルートジョブネットの情報のみ出力 |
| `-f フォーマット` | 出力フォーマットを指定（フォーマット指示子） |
| `-l` | 詳細形式で出力 |
| `-b 日付` | 基準日を指定 |
| `-e 日付` | 終了日を指定 |
...
ajsshowコマンドの`%rr`フォーマット指示子で取得できます：
```cmd
ajsshow -F AJSROOT1 -g 1 -i '%## %rr %ll %I' /main_unit/jobgroup1/daily_batch/job1
```
