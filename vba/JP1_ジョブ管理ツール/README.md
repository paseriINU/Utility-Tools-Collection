...
```
JP1_ジョブ管理ツール/
├── JP1_JobManager_Setup.bas  # 初期化（シート作成・フォーマット）
├── JP1_JobManager.bas        # メイン処理（ジョブ取得・実行）
└── README.md
```
...
3. 設定シート、ジョブ一覧シート、実行ログシートが作成される
...
- **選択列**: チェックボックスをクリックして選択します
- **順序列**: チェックを入れた順番で自動的に数字が振られます
- チェックを外すと順序が再採番されます
- **順序の手動変更**: 順序列は手動でも変更可能です（1から連番で入力）
- **保留中のジョブ**: オレンジ色でハイライト表示されます
- **フィルター使用時**: シートをアクティブにすると自動的にチェックボックスが再配置されます

```
| 選択 | 順序 | ジョブネットパス                    | ジョブネット名    | 保留   |
|:----:|:----:|--------------------------------------|-------------------|--------|
...
**操作方法**:
1. 実行したいジョブの「選択」列（A列）のチェックボックスをクリック
2. チェックが入り、順序が自動的に振られます
3. チェックを外すには再度クリック（順序も自動で再採番）
4. 順序を変更したい場合は、順序列（B列）に直接数字を入力
...
| 列 | 内容 |
|----|------|
| 選択 | チェックボックスをクリックして選択 |
| 順序 | 実行順（チェックした順番で自動採番） |
| 種別 | グループ/ジョブネット/ジョブ |
| ユニットパス | ジョブネットのフルパス |
| ユニット名 | ジョブネット名（ajsprintから取得） |
| コメント | ジョブネットのコメント |
| 保留 | 保留中の場合「保留中」と表示（行もオレンジ色でハイライト） |
| 最終実行結果 | 正常終了/異常終了/起動失敗/保留解除失敗 |
| 開始時刻 | 最終実行の開始時刻 |
| 終了時刻 | 最終実行の終了時刻 |
| ログパス | 実行ログファイルへのハイパーリンク |

**ボタン**:
- **選択ジョブ実行**: 選択したジョブを順序どおりに実行
- **一覧クリア**: ジョブ一覧をクリア
...
→ 設定シートの必須項目（JP1サーバ、リモートユーザー、JP1ユーザー）を入力してください。
