...
メインシートで以下のオプションを設定できます：

- **厳密比較（LCS）を使用する**: チェックすると、LCSアルゴリズムで厳密な差分検出を行います。大規模な構造変更がある文書の比較に適しています。デフォルトはオフ（簡易比較・高速）
- **スタイル変更も検出する**: チェックすると、テキスト内容が同じでもフォント、サイズ、太字/斜体などのスタイル変更を検出します。デフォルトはオン
...
- **2026-01-06**: スタイル比較オプション追加
  - メインシートに「スタイル変更も検出する」チェックボックスを追加
  - チェックをオフにするとスタイル比較をスキップし、テキストの変更のみを検出
  - 大量の段落がある文書で処理速度を向上させたい場合に有効
