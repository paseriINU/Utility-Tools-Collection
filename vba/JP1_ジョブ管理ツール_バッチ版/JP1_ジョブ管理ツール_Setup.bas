...
Public Sub InitializeJP1Manager()
    ...
    MsgBox "初期化が完了しました。" & vbCrLf & vbCrLf & _
           "【バッチ版の注意】" & vbCrLf & _
           "JP1_ジョブ実行.bat をこのExcelブックと同じフォルダに配置してください。" & vbCrLf & vbCrLf & _
           "1. メインシートで接続設定を入力してください" & vbCrLf & _
           "2. 「ジョブ一覧取得」ボタンでジョブを取得" & vbCrLf & _
           "3. ジョブ一覧シートで順序を設定" & vbCrLf & _
           "4. 「選択ジョブ実行」ボタンで実行", _
           vbInformation, "JP1 ジョブ管理ツール（バッチ版）"
End Sub
...
Private Sub FormatMainSheet()
    ...
    ' 説明
    ws.Range("A2").Value = "JP1サーバに接続してジョブネット一覧を取得し、選択したジョブを実行します。（外部バッチファイル使用版）"
