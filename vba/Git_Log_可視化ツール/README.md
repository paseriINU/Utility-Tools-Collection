...
| C | フルハッシュ | コミットハッシュ（完全版40文字） |
| D | 作者 | コミット作成者 |
| E | メール | コミット作成者のメールアドレス |
| F | 日時 | コミット日時 |
| G | 曜日 | コミット日の曜日 |
| H | 種別 | コミット種別（初期/マージ/通常） |
| I | コミットメッセージ | コミットの件名 |
| J | ブランチ/タグ | 該当するブランチ・タグ名 |
| K | 変更ファイル数 | 変更されたファイル数 |
| L | 追加行 | 追加された行数 |
| M | 削除行 | 削除された行数 |
| N | 変更量 | 追加行 + 削除行の合計 |
| O | 親コミット数 | 親コミットの数（マージ判定に使用） |

**行の色分け:**
- **赤色背景**: 初期コミット（親コミットなし）
- **緑色背景**: マージコミット（親コミット2つ以上）
- **交互グレー**: 通常コミット（見やすさのため）

**機能:**
- フィルター機能で作者やブランチで絞り込み可能
- ソート機能で日時順や変更量順に並べ替え可能
...
3つのシートが作成され、以下の情報が表示されます:

- **メイン**: 設定入力欄と実行ボタン
- **コミット履歴**: 最近100件のコミット一覧表（15列の詳細情報）
...
- **既存シート上書き**: 実行するたびに3つのシート（メイン、コミット履歴、ブランチグラフ）が上書きされます
...
- **2025-12-17**: 大幅リニューアル
  ...
  - コミット履歴シートを15列に拡張（フルハッシュ、メール、曜日、種別、変更量、親コミット数を追加）
