...
3. 以下の2つのモジュールを順番にインポート：
   - `Git_Log_可視化ツール_Setup.bas`（初期化・シートフォーマット用）
   - `Git_Log_可視化ツール.bas`（メイン処理用）
4. 標準モジュール「Git_Log_可視化ツール_Setup」と「Git_Log_可視化ツール」が追加されたことを確認

### 3. 初期化を実行

1. `Alt + F8` を押してマクロ一覧を表示
2. `InitializeGitLog可視化ツール` を選択して実行
