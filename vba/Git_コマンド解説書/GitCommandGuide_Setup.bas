Attribute VB_Name = "GitCommandGuide_Setup"
Option Explicit

'==============================================================================
' Git コマンド解説書 生成ツール - 初期化モジュール
'   - シート作成・フォーマット処理
'   - このツールは初期化のみで、業務ロジックは含まれません
'==============================================================================
' 注意: このツールは解説書を生成するだけのツールのため、
'       すべての処理がこのSetupモジュールに含まれています。
'       メインモジュール (GitCommandGuide.bas) は互換性のために残されています。
'==============================================================================
...
Private Sub FormatBasicsSheet()
    ...
    With ws
        ...
        .Range("A4:H4").Merge
        .Range("A5").Value = "プログラムのソースコードだけでなく、ドキュメントや設定ファイルなど、あらゆるテキストファイルの管理に使います。"
        .Range("A5:H5").Merge
        ...
        reasons = Array( _
            "1. 変更履歴が残る - いつ、誰が、何を変更したか記録される", _
            "2. 過去に戻れる - 間違えても以前の状態に戻せる", _
            "3. 複数人で作業できる - チームで同時に開発できる", _
            "4. 分岐しやすい - ブランチで安全に新機能を開発", _
        ...
        terms = Array( _
            ...
            Array("Commit", "コミット", "変更を記録すること。セーブポイントをつくるイメージ。"), _
            Array("Branch", "ブランチ", "開発の分岐。メインとは別の作業場所を作る。"), _
            ...
            Array("Origin", "オリジン", "リモートリポジトリのデフォルト名。通常はGitHubを指す。") _
        )
        ...
        .Range("A35").Value = "(編集)         (ステージ)  (ローカル保存)  (リモート送信)"
...
Private Sub FormatBasicCommandsSheet()
    ...
    With ws
        ...
        commands = Array( _
            ...
            Array("git status", "現在の状態を確認", "git status", "迷ったらこれ! 一番よく使う"), _
        ...
        Dim tips As Variant
        tips = Array( _
            "・迷ったら git status を実行! 何が起きているかわかります", _
...
Private Sub FormatBranchSheet()
    ...
    With ws
        ...
        .Range("A4").Value = "ブランチは「枝分かれ」という意味。メインの開発ラインとは別に、独立した作業場所を作ります。"
        .Range("A4:H4").Merge
        .Range("A5").Value = "新機能の開発やバグ修正を、メインに影響を与えずに安心して行えます。"
        .Range("A5:H5").Merge
...
Private Sub FormatRemoteSheet()
    ...
    With ws
        ...
        .Range("A25:H25").Merge
        .Range("A26").Value = ""
        .Range("A27").Value = "安全に確認したいなら fetch → 内容確認 → merge の流れが安心です"
...
Private Sub FormatHistorySheet()
    ...
    With ws
        ...
        otherCommands = Array( _
            ...
            Array("git blame ファイル名", "各行を誰が変更したか表示"), _
...
Private Sub FormatUndoSheet()
    ...
    With ws
        ...
        ' 警告
        .Range("A3").Value = "注意: これらのコマンドは変更を消します。慎重に使ってください!"
        .Range("A3").Font.Bold = True
        ...
        undoCommands = Array( _
            Array("ファイルの変更を元に戻したい", "git checkout -- ファイル名", "最後のコミット状態に戻る"), _
            Array("全ファイルの変更を元に戻したい", "git checkout -- .", "全ファイルを戻す（危険）"), _
            Array("git addを取り消したい", "git reset HEAD ファイル名", "ステージングを解除"), _
            Array("git addを全て取り消したい", "git reset HEAD", "全てのステージングを解除") _
        )
        ...
        commitCommands = Array( _
            Array("直前のコミットメッセージ修正", "git commit --amend", "エディタが開く"), _
            Array("直前のコミットに追加で変更を含める", "git add ファイル名" & vbLf & "git commit --amend --no-edit", "メッセージは変わらない"), _
            Array("直前のコミットを取り消す(変更は残る)", "git reset --soft HEAD^", "ステージング状態に戻る"), _
...
Private Sub FormatScenarioSheet()
    ...
    With ws
        ...
        ' シナリオ4
        .Range("A" & row).Value = "シナリオ4: 毎日の作業開始"
        .Range("A" & row).Font.Size = 14
        .Range("A" & row).Font.Bold = True
...
Private Sub FormatTroubleshootSheet()
    ...
    With ws
        ...
        troubles = Array( _
            ...
            Array("コンフリクト（競合）が起きた", "同じ箇所を別々に変更した", "ファイルを編集して解決後 git add → git commit"), _
            Array("間違えてコミットした", "コミット内容に誤り", "git commit --amend（直前）または git revert"), _
            Array("変更が消えてしまった", "git reset --hard を実行", "git reflog でコミットIDを探して git checkout"), _
            Array("ブランチを間違えた", "別のブランチで作業していた", "git stash → git checkout → git stash pop"), _
            ...
            Array("大きなファイルをpushできない", "GitHubの容量制限", "git reset HEAD^ でコミット取消、.gitignoreに追加"), _
            Array("日本語ファイル名が文字化け", "文字コード設定", "git config --global core.quotepath false"), _
            Array("改行コードの警告が出る", "LF/CRLF問題", "git config --global core.autocrlf true (Windows)") _
        )
