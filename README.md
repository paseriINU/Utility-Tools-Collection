...
│   ├── JP1_ジョブログ取得ツール/  # JP1ジョブスプール取得ツール
│   ├── JP1_ジョブツール/          # JP1ジョブ起動+ログ取得（統合版）
│   ├── JP1_REST_ジョブ情報取得ツール/ # JP1 REST APIでジョブ情報取得
│   ├── JP1_REST_ジョブ実行ログ取得ツール/ # JP1 REST APIでジョブ実行＋ログ取得
...
- **[JP1 ジョブログ取得ツール](batch/JP1_ジョブログ取得ツール/)**: JP1/AJS3ジョブのスプールを取得
  - 指定ジョブの標準出力/標準エラー出力をクリップボードにコピー
  - ajsshow -B コマンドでスプールを取得
  - 事前に設定ファイルでジョブパスを定義
  - ダブルクリックで実行可能なスタンドアローン版

- **[JP1 ジョブツール](batch/JP1_ジョブツール/)**: JP1ジョブネット起動とログ取得の統合ツール
  - ジョブネット起動 → 完了待ち → ログ取得を連続実行
  - ログをクリップボードに自動コピー
  - ダブルクリックで実行可能
...
- **[JP1 REST API ジョブ実行ログ取得ツール](batch/JP1_REST_ジョブ実行ログ取得ツール/)**: JP1/AJS3 Web Console REST APIでジョブを即時実行しログを取得
  - ジョブパスを指定するとルートジョブネットを自動特定して即時実行
  - 完了待ち機能（タイムアウト設定可能）
  - 完了後に実行結果詳細を自動取得
  - WinRM不要（Web Console経由）
  - サンプルバッチ付き（実行確認・ファイル保存）
