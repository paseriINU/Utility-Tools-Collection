...
│   ├── Git_差分抽出ツール/       # Git差分ファイル抽出ツール（バッチ版のVBA版）
│   └── Excel_フィルター検索ツール/ # 複数キーワードOR条件フィルター検索
│
...
└── javascript/              # JavaScriptツール
    ...
    └── AI_レクチャー資料/     # Claude Code実践ガイド（reveal.jsスライド）
```
...
- **[Git リモートブランチリネームツール](batch/Git_リモートブランチリネームツール/)**: リモートブランチ名を変更
  - 一覧から選択または直接入力でリネーム
  - ローカルブランチも同時にリネーム
  - 他の開発者への影響警告を表示
  - 保護ブランチ（main/master/develop/release）はリネーム不可
  - ダブルクリックで実行可能
...
- **[Git 空フォルダ管理ツール](batch/Git_空フォルダ管理ツール/)**: 空フォルダに.gitignoreを作成してGit管理
  - 再帰的にサブフォルダを検索して空フォルダを検出
  - .gitフォルダ、node_modules等は自動除外
  - 除外パターンのカスタマイズ可能
  - 既存の.gitignoreはスキップ
  - ダブルクリックで実行可能
...
- **[JP1 ジョブツール v2](batch/JP1_ジョブツール_v2/)**: JP1ジョブツール改善版
  - v1からの改善: 出力オプション引数を追加（/NOTEPAD、/EXCEL、/WINMERGE）
...
- **[PDF テキスト比較ツール](batch/PDFテキスト比較ツール/)**: 2つのPDFをテキスト比較するGUIツール
  - ドラッグ&ドロップでPDFファイルを選択
  - クリックでファイル選択ダイアログも使用可能
  - WordでPDFを開いてテキスト抽出
  - WinMergeで差分比較を自動起動
  - ダブルクリックで実行可能

- **[makeファイル検証ツール](batch/makeファイル検証ツール/)**: makeファイル内の.oファイル参照を検証
  - makeファイル名と.oファイル名の一致をチェック
  - パス付きの.oファイル参照にも対応
  - 例外ファイルの指定が可能（複数指定可）
  - 部分一致モードのサポート
  - ダブルクリックで実行可能
...
- **[Git 差分抽出ツール](vba/Git_差分抽出ツール/)**: Gitブランチ間の差分ファイル抽出（バッチ版のVBA版）
  - Gitブランチ/コミット間の差分ファイルを検出
  - 比較結果をExcelシートに出力（履歴として保存可能）
  - 差分ファイル一覧を色分け表示（変更/追加/削除）
  - 差分ファイルをフォルダ構成維持でコピー
  - 修正前/修正後のファイルを分けて出力

- **[Excel フィルター検索ツール](vba/Excel_フィルター検索ツール/)**: 複数キーワードでOR条件フィルター検索
...
- **[AI レクチャー資料](javascript/AI_レクチャー資料/)**: Claude Code実践ガイド（プログラマー向け入門スライド）
  - reveal.jsベースのHTMLスライド（35枚）
  - プログラミング経験者向けのClaude Code入門資料
  - 効果的なプロンプトの書き方、実践的な活用シーンを解説
  - ブラウザで開くだけで表示可能（インストール不要）
  - ダークテーマで視認性に配慮
