...
│   └── WinMergeフィルター設定ツール/  # WinMerge除外フィルター設定
│
├── linux/                   # Linuxスクリプト
...
- **[WinMergeフィルター設定ツール](batch/WinMergeフィルター設定ツール/)**: WinMergeにGit管理ファイル除外フィルターを組み込み
  - .gitignore, .gitkeep, .gitattributes等を比較から除外
  - .fltファイル同梱、ワンクリックでインストール
  - .gitフォルダも除外対象
  - ダブルクリックで実行可能
